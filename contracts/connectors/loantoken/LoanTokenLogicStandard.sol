...
contract LoanTokenLogicStandard is LoanTokenSettingsLowerAdmin {
    ...
    */
	...
	function _totalDeposit(
		...
		if (collateralTokenSent != 0) {
			...
			if (sourceToDestPrecision != 0) {
				totalDeposit = collateralTokenSent.mul(sourceToDestRate).div(sourceToDestPrecision).add(totalDeposit);
			}
		}
	}
		...
		returns (uint256 borrowAmount, uint256 interestRate)
	{
		...
		interestRate = _nextBorrowInterestRate2(depositAmount.mul(10**20).div(initialMargin), _totalAssetSupply(0));
